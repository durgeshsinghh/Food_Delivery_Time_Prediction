...
def data_cleaning(data: pd.DataFrame):
    ...
    # FIXED: ratings is float64, not string
    six_star_data = data.loc[data['ratings'].astype(float) >= 6]
...
def perform_data_cleaning(data: pd.DataFrame, saved_data_path="Food_time_cleaned.csv"):
...
if __name__ == "__main__":
    DATA_PATH = "India-Food-Delivery-Time-Prediction.txt"

    df = pd.read_json(DATA_PATH)
    print("data loaded successfully")

    df_final = perform_data_cleaning(df)
